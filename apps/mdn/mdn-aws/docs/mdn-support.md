...
> MDN is running on the IT Portland Kubernetes cluster, if you don't have credentials to access the cluster, reach out to Ed Lim
...
- [MDN CDN resource definition](https://github.com/mdn/infra/tree/master/apps/mdn/mdn-aws/infra/modules/mdn-cdn)
...
```
kubectl -n mdn-prod scale --replicas=20 deploy web
```

Check rolling update status:

```
kubectl -n mdn-prod rollout status deploy web
```
...
To determine the K8s client and server versions:
...
The [rds_backup_tool](https://github.com/mdn/infra/blob/master/tools/aws/rds_backup_tool/README.md) takes full mdn-prod encrypted backups that are stored in S3 (and moved to Glacier after 30 days). To access and decrypt a backup, follow [these](https://github.com/mdn/infra/blob/master/tools/aws/rds_backup_tool/README.md#decrypting-a-database-archive) instructions.
